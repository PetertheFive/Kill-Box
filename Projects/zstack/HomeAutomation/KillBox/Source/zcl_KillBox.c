...
void zclSampleLight_Init( byte task_id )
{
  ...
  // Initialize the UV control
  P0SEL &= ~(BV(4));    /* Set pin function to GPIO */
  P0DIR |= BV(4);       /* Set pin direction to Output */
  
  // Automatically turn on the UV
...
uint16 zclSampleLight_event_loop( uint8 task_id, uint16 events )
{
  ...
  /* Turn off the UV automatically */
  if ( events & SAMPLELIGHT_TURNOFF_UV_EVT )
  {   
    osal_stop_timerEx( zclSampleLight_TaskID, SAMPLELIGHT_TURNOFF_UV_EVT );
    zclSampleLight_OnOff = LIGHT_OFF;
    UV_Auto_Turn_Off = 1;
    zclSampleLight_LcdDisplayUpdate();
    return ( events ^ SAMPLELIGHT_TURNOFF_UV_EVT );
  }  
...
static void zclSampleLight_DisplayLight( void )
{
  ...
  if ( zclSampleLight_OnOff == LIGHT_ON )
  {
    ...
    osal_start_timerEx( zclSampleLight_TaskID, SAMPLELIGHT_TURNOFF_UV_EVT, 60000 );    
  }
  else
  {
...
static void zclSampleLight_OnOffCB( uint8 cmd )
{
  ...
  // Turn on the light
  if (( cmd == COMMAND_ON )&&( !HAL_PUSH_BUTTON2() ))
  {
    zclSampleLight_OnOff = LIGHT_ON;
  }
  // Turn off the light
